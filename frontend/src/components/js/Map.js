  ...
  methods: {
    ...
    async loadGLBModels() {
      try {
        ...
        for (const item of models) {
          const position = Cartesian3.fromDegrees(item.lon, item.lat, item.height);
    ...
    async initCesium() {
      ...
      this.viewer = new Viewer("cesiumContainer", {
        terrainProvider: await CesiumTerrainProvider.fromUrl(
          "http://localhost:2004/tilesets/tiles"
        ),
        animation: false,
        timeline: false,
    ...
    setupMeasureButton() {
      ...
      btnMeasure.addEventListener('click', (e) => {
        e.stopPropagation();
        e.preventDefault();
        if (panelMeasure.style.display === 'none' || panelMeasure.style.display === '') {
    ...
    toggleHeightMeasure() {
      ...
      if (this.measureActive) {
        ...
        this.showNotification("Chế độ đo chiều cao đã bật. Click 2 điểm để đo Δh.", "info");
      }
    },
    ...
    handleHeightMouseMove(movement) {
      ...
      this.dynamicMeasureLine.polyline.positions = [this.firstMeasurePoint, pickedPos];
    },
    ...
    toggleLocatePoint() {
      ...
      if (this.locateActive) {
        ...
        this.showNotification("Chế độ lấy tọa độ đã bật. Click vào bản đồ!", "info");
      }
    },

    activateLocatePoint() {
      this.locateHandler = new ScreenSpaceEventHandler(this.viewer.scene.canvas);
    ...
    addCoordinateMarker(position, lat, lon, height) {
      ...
      const marker = this.viewer.entities.add({
        ...
        label: {
          text: `📍 ${this.coordMarkers.length + 1}\nLat: ${lat}°\nLon: ${lon}°\nH: ${height}m`,
          font: "14px sans-serif",
          showBackground: true,
          backgroundColor: Color.BLACK.withAlpha(0.7),
          fillColor: Color.YELLOW,
        ...
        description: `Tọa độ điểm ${this.coordMarkers.length + 1}: ${lat}°, ${lon}°, ${height}m`
      });
    ...
    toggleAttr() {
      ...
      this.attrHandler.setInputAction(
        (click) => {
          ...
          if (defined(picked) && picked.getProperty) {
            ...
            this.attrContent = rows || "<tr><td colspan='2'>Không có thuộc tính</td></tr>";
          }
        },
        ScreenSpaceEventType.LEFT_CLICK
      );
